...
then
   ...
   mysql --socket=/run/mysqld/mysqld.sock --host=localhost -uroot -e "CREATE DATABASE IF NOT EXISTS \`$MYSQL_DATABASE_NAME\`;"
   mysql --socket=/run/mysqld/mysqld.sock --host=localhost -uroot -e "CREATE USER IF NOT EXISTS \`$MYSQL_USERNAME\`@'%';"
   mysql --socket=/run/mysqld/mysqld.sock --host=localhost -uroot -e "GRANT ALL PRIVILEGES ON *.* TO \`$MYSQL_USERNAME\`@'%' IDENTIFIED BY '$MYSQL_PASSWORD';"
   mysql --socket=/run/mysqld/mysqld.sock --host=localhost -uroot -e "FLUSH PRIVILEGES ; "
